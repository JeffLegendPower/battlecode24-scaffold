...
import static BETA.v1_BETA.Util.getClosest;
import static BETA.v1_BETA.Util.getFurthest;
...
public class Attacker {
    ...
    public static void tick(RobotController rc, MapLocation curLoc) throws GameActionException {
        ...
        for (RobotInfo ally : allyRobots) {
            if (ally.hasFlag) {
                flagHolder = ally;
                Pathfinding.moveTowards(rc, curLoc, flagHolder.getLocation().add(RobotPlayer.directions[RobotPlayer.rng.nextInt(8)]).add(RobotPlayer.directions[RobotPlayer.rng.nextInt(8)]), true);
            }
         }
...
//            moveTowards(rc, curLoc, getClosest(rc.getAllySpawnLocations(), curLoc), 3);
            Pathfinding.moveTowards(rc, curLoc, Util.getClosest(rc.getAllySpawnLocations(), curLoc), true);
        }
            ...
            if (rc.getHealth() < 300)
                Pathfinding.moveTowards(rc, curLoc, curLoc.add(curLoc.directionTo(nearestEnemy.getLocation()).opposite()), true);
            ...
            if (dist < 9 || dist > 16) // If we move forward to attack they will get the first hit
                Pathfinding.moveTowards(rc, curLoc, nearestEnemy.getLocation(), true); // Try to move towards the nearest enemy
