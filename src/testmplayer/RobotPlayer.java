...
public class RobotPlayer {
    ...
    public static void onGameTurn() throws GameActionException {
        ...
        if (isCarrier) {
            ...
            for (MapLocation adjacent : sort(getAdjacents(robotLoc), (loc) -> {
                ...
                for (MapLocation trapLocation : trapLocations) {
                    ...
                    total -= trapLocation.distanceSquaredTo(loc) / 3;
                }
                for (MapLocation allyLocation : allyLocations) {
                    total -= allyLocation.distanceSquaredTo(loc) / 3;
                }
                return loc.distanceSquaredTo(carrierDestination) - total;
            })) {
                if (flagHolderVisited.contains(adjacent)) {
                    continue;
                }
                ...
                if (rc.canMove(d)) {
                    ...
                    flagHolderVisited.add(adjacent);
                    writeLocationToShared(4+flagCarrierIndex, adjacent, 0, 0);
                    break;
                }
            }
            // pass the flag if stuck
            if (turnsSinceLastMove > 7 || (rc.getMovementCooldownTurns() < 10 && rc.getActionCooldownTurns() < 10)) {
        ...
        if (turnsSinceLastMove > 10 && rc.getMovementCooldownTurns() <= 20) {
            Direction tryDir = directions[rng.nextInt(8)];
            while (lastTryDir != null && tryDir == lastTryDir) {
                tryDir = directions[rng.nextInt(8)];
            }
            if (rc.canMove(tryDir)) {
                rc.move(tryDir);
                turnsSinceLastMove -= 5;
                turnsSinceLastMove = Math.max(turnsSinceLastMove, 0);
                lastTryDir = tryDir;
            }
        } else {
            lastTryDir = null;
        ...
        for (FlagInfo flagInfo : rc.senseNearbyFlags(-1, rc.getTeam().opponent())) {
            ...
            if (flagInfo.getLocation().distanceSquaredTo(robotLoc) <= 2) {  // grab the flag
                ...
                if (rc.canPickupFlag(flagInfo.getLocation())) {
                    ...
                    flagHolderVisited = new ArrayList<>();
                    flagHolderVisited.add(robotLoc);
                    for (int i=0; i<3; i++) {
                        ...
                        if (carrierLocations[i].distanceSquaredTo(flagInfo.getLocation()) <= 1) {  // picked up dead carrier's flag
                            flagCarrierIndex = i;
        ...
        if (enemyInfos.length == 0 || enemyInfos[0].getLocation().distanceSquaredTo(robotLoc) >= 16) {
            ...
            if (allyInfos.length > 0) {
                ...
                RobotInfo weakestAlly = getWeakest(allyInfos);
                if (healTarget == null) {
                    if (weakestAlly.getHealth() <= 1000 - rc.getHealAmount()) {
                        healTarget = weakestAlly;
                    } else {
                        healTarget = closestAlly;
                    }
                }
                if (healTarget.getHealth() <= 1000 - rc.getHealAmount()) {
                    for (Direction d : sort(
                            getIdealMovementDirections(robotLoc, pathfindGoalLoc),
                            (d) -> robotLoc.add(d).distanceSquaredTo(healTarget.getLocation()))
                    ) {
                        if (rc.canMove(d)) {
                            rc.move(d);
                            turnsSinceLastMove = 0;
        ...
        RobotInfo weakestEnemy = getWeakest(enemyInfos);
        if (weakestEnemy.getHealth() <= allyInfos.length * rc.getAttackDamage()) {
            attackTarget = weakestEnemy.getLocation();
        } else {
            attackTarget = closestEnemy.getLocation();
        }
        if (allyInfos.length >= enemyInfos.length-3) {  // more allies than enemies, attack
            int distToTarget = robotLoc.distanceSquaredTo(attackTarget);

            // 1+ steps away, so move then attack
            if (distToTarget > 4) {
                for (Direction d : getIdealMovementDirections(robotLoc, attackTarget)) {
                    if (rc.canMove(d)) {
                        rc.move(d);
                        turnsSinceLastMove = 0;
            ...
            if (distToTarget <= 4) {
                if (rc.canAttack(attackTarget)) {
                    rc.attack(attackTarget);
                    for (Direction d : getIdealMovementDirections(attackTarget, robotLoc)) {
                        if (rc.canMove(d)) {
                            rc.move(d);
                            turnsSinceLastMove = 0;
                            break;
        ...
        if (attackTarget.distanceSquaredTo(robotLoc) >= 16) {  // can safely flee
            MapLocation finalPathfindGoalLoc = pathfindGoalLoc;
            for (Direction d : sort(getIdealMovementDirections(attackTarget, robotLoc), (dir) -> robotLoc.add(dir).distanceSquaredTo(finalPathfindGoalLoc))) {
                if (rc.canMove(d)) {
                    rc.move(d);
                    turnsSinceLastMove = 0;
...
//                    robotLoc.add(Direction.NORTHWEST), robotLoc.add(Direction.SOUTHEAST),
//                    robotLoc.add(Direction.SOUTHWEST), robotLoc.add(Direction.NORTHEAST)}) {
            for (MapLocation diag : getAdjacents(robotLoc)) {
                if (rc.canBuild(TrapType.STUN, diag)) {
                    rc.build(TrapType.STUN, diag);
                }
            }
